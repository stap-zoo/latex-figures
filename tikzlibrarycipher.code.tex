...
\newcommand{\MatrixStateMN}[3]{%
  \tikz[stateopts]{
    % #1: rows (m), #2: cols (n), #3: TikZ content using ss<i><j>
    \pgfmathtruncatemacro{\lastrow}{#1 - 1}
    \pgfmathtruncatemacro{\lastcol}{#2 - 1}
    \foreach \i in {0,...,\lastrow} {
      \foreach \j in {0,...,\lastcol} {
        \coordinate[overlay] (ss\i\j) at (\j+.5,-\i-.5);
      }
    }
    \coordinate (label) at (#2/2, -#1/2);
    #3
...
\newcommand{\FillH}[3][fillopts]{\foreach \j in {#3} {\FillCell[#1]{ss#2\j}}} % #2=row, #3=columns

\newcommand{\FillV}[3][fillopts]{\foreach \i in {#3} {\FillCell[#1]{ss\i#2}}} % #2=column, #3=rows
